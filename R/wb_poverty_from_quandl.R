#' Download World Bank Poverty Data From Quandl
#'
#' A wrapper around \code{Quandl}. Download Poverty data covered by the World Bank with just one line of code.
#'
...
#'}
#'
#' For any of those calls the \code{wb_poverty_from_quandl()} will download data for all the countries in the requested region. A complete region list can be seen at: \url{https://www.imf.org/external/pubs/ft/weo/2018/01/weodata/groups.htm}.
#'
...
#'     theme_classic()
#'}
wb_poverty_from_quandl <- function(countries, indicators, ...) {
  ...
  database <- wbi_indicators[['result']] %>%
    ...
    dplyr::mutate(quandl_code = stringr::str_c('WPOV/', iso, '_', CODE))
