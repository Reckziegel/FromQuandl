#' Download World Bank Governance Data From Quandl
#'
#' A wrapper around \code{Quandl}. Download governance data covered by the World Bank with just one line of code.
#'
...
#'}
#'
#' For any of those calls the \code{wb_governance_from_quandl()} will download data for all the countries in the requested region. A complete region list can be seen at: \url{https://www.imf.org/external/pubs/ft/weo/2018/01/weodata/groups.htm}.
#'
...
#' # in Latin America
#' wb_governance_from_quandl('latam', 'VA_NO_SRC')
#'
wb_governance_from_quandl <- function(countries, indicators, ...) {
  ...
  database <- wbi_indicators[['result']] %>%
    ...
    dplyr::mutate(quandl_code = stringr::str_c('WWGI/', iso, '_', CODE))
  ...
  database %>%
    ...
    dplyr::rename(date = 'Date', value = 'Value', indicator = 'INDICATOR') %>%
